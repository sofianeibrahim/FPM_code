function [O_rec, phase_rec, P_rec, err_curve] = Algorithm_QN1(ImagesIn, LED_Array, systemSetup, options, centerTag)
...
for it = 1:options.maxIter
    ...
    for m = 1:Nimgs
        ...
        nX = idx_X(m);
        nY = idx_Y(m);
        cen = cen0 + [nY, nX];                % center in big canvas

        % crop subregion from O (ensure indices integer)
        n1r = cen(1) - floor(Ny/2); n2r = n1r + Ny - 1;
        n1c = cen(2) - floor(Nx/2); n2c = n1c + Nx - 1;
        % guard boundaries (should not happen if N_obj chosen large enough)
        if n1r < 1 || n1c < 1 || n2r > N_objY || n2c > N_objX
            error('Subregion indices out of bounds. Increase N_obj or check idx_X/idx_Y.');
        end

        Psi0 = O(n1r:n2r, n1c:n2c) .* P;   % frequency subregion * pupil
        psi0 = IFT(Psi0);                  % estimated low-res field
        I_est = abs(psi0).^2;
        
        % measured image: assume stack order matches LED list
        I_mea = ImagesIn(:,:,m);

        end
        % intensity correction update (after first iter)
        if it > 1 && options.IntCorr == 1
            % scalar factor for this LED cell
            disp(m)
            c(m) = sum(sum(sqrt(I_est))) / (sum(sum(sqrt(I_mea))));
            
        end

        % forward projection (amplitude replace)
        Psi = forward_project(psi0, I_mea * c(m), I_est, FT);
        dPsi = (Psi - Psi0);

        % update O and P (Quasi-Newton / GDUpdate)
        [O, P] = GDUpdate_rank1(O, P, dPsi, max(abs(O(:))), cen, pupil0, options.alpha, options.beta);

        % measure error for this LED
        err_iter(m) = rms(I_mea(:) - I_est(:));
        if it > 1 && options.IntCorr == 1
            ImagesIn(:,:,m) = ImagesIn(:,:,m) .* c(m);
        end
        % [idx_X, idx_Y] = LEDsPosCorrection(O, P, I_mea, m, idx_X, idx_Y, cen0, mx, options, c,Ny, Nx,IFT);
    end
